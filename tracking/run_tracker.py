...
def calc_iou_graph(iou):
    threshold = np.linspace(0, 1, 50)
    graph = []

    for t in threshold:
        c = (iou > t).sum()
        graph.append(c / float(len(iou)))

    graph = np.array(graph)
    return threshold, graph
...
def run_mdnet(img_list, init_bbox, gt=None, savefig_dir='', display=False):
    ...
    for i in range(len(img_list)):
        ...
        if gt is None:
            ...
            overlab = overlap_ratio(gt[i],result_bb[i])[0]
            print "Frame %d/%d, Overlap %.3f, Score %.3f, Time %.3f" % \
                (i+1, len(img_list), overlab, target_score, spf)
            iou.append(overlab)

    iou = np.array(iou)
    fps = len(img_list) / spf_total
    return result, result_bb, fps, iou
...
if __name__ == "__main__":
    ...
    img_list, init_bbox, gt, savefig_dir, display, result_path = gen_config("../dataset/OTB", args.seq, args.savefig, args.display)
